	...
	if use_simulated_data:
		...
		for i in range(expected_size):
			...
			if segment['name'] == 'tip':
				...
				center_dist = ((i % width - width/2)**2 + (i // width - height/2)**2)**0.5
				val = int(800 * np.exp(-center_dist) * (0.7 + 0.3 * np.sin(frame * 0.1)))
			elif segment['name'] == 'pad':
				# Pads have wave patterns
				val = int(500 + 400 * np.sin(i * 0.2 + frame * 0.1) * np.cos(i * 0.1 + frame * 0.08))
			elif segment['name'] == 'middle':
				# Middle section has circular patterns
				center_dist = ((i % width - width/2)**2 + (i // width - height/2)**2)**0.5
				val = int(600 * np.exp(-center_dist/2) * (0.8 + 0.2 * np.cos(frame * 0.15)))
			else:  # base or palm
				# Base and palm have random-like patterns
				val = int(300 + 200 * np.sin(i * 0.3 + frame * 0.05) * np.sin(i * 0.2 + frame * 0.07))
...
class TouchDataVisualizer:
	def __init__(self, use_simulated_data=False):
		...
		if not use_simulated_data:
			try:
				...
				if not connection_status:
					print(f"Failed to connect to Modbus device at {MODBUS_IP}:{MODBUS_PORT}")
		...
		for col, finger in enumerate(['thumb', 'index', 'middle', 'ring', 'pinky']):
			plt.figtext(0.1 + col * 0.16, 0.95, FINGER_LABELS[finger], 
	...
	def _init_finger_visualizations(self):
		...
		for finger, segments in FINGER_SEGMENTS.items():
			...
			if finger == 'palm':
				...
				ax = self.fig.add_subplot(self.grid[3, 1:4])  # Bottom center, span 3 columns
				segment = segments[0]  # Palm has only one segment
				dims = segment['dims']
				empty_data = np.zeros(dims)
				img = ax.imshow(empty_data, cmap=self.cmap, vmin=0, vmax=1023,
	...
	def update_plot(self, frame):
		...
		try:
			...
			for finger, segments in FINGER_SEGMENTS.items():
				for segment in segments:
					...
					if segment_data:
						...
						ax.set_xlabel(f'Min: {min_val} | Max: {max_val} | Avg: {avg_val:.1f}')
			...
			updated_artists.extend([self.timestamp_text, self.frequency_text, 
...
if __name__ == "__main__":
	...
	parser.add_argument('--simulate', action='store_true', help='Use simulated data instead of real sensor data')
