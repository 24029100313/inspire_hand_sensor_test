...
class InspireHandController:
    ...
    ACTION_DELAY = 0.04  # seconds
    ACTION_TIMEOUT = 0.14  # seconds

    def __init__(
        self, ip="192.168.11.210", port=6000, external_client=None, external_lock=None
    ):
        """
        ...
        )  # Flag to track if client is external
            ...
            for i, pos in enumerate(positions):
                if pos != -1:  # -1 means no change
                    if pos < self.MIN_POSITION or pos > self.MAX_POSITION:
                        print(
                            f"Position {i} ({pos}) out of range [{self.MIN_POSITION}, {self.MAX_POSITION}]"
                        )
                        return False
            ...
            for i, speed in enumerate(speeds):
                if speed != -1:  # -1 means no change
                    if speed < self.MIN_SPEED or speed > self.MAX_SPEED:
                        print(
                            f"Speed {i} ({speed}) out of range [{self.MIN_SPEED}, {self.MAX_SPEED}]"
                        )
                        return False
    ...
    def _wait_for_action_completion(self, timeout=None):
        ...
        while time.time() - start_time < timeout:
            ...
            for i, (current, target) in enumerate(
                zip(current_angles, self.target_angles)
            ):
        ...
        for i, pos in enumerate(positions):
            if pos != -1:  # Only check positions that are not already -1
                ...
                if abs(pos - current_angles[i]) < 20:
                    ...
                    print(
                        f"Skipping finger {i} (change too small: {abs(pos - current_angles[i])})"
                    )
                else:
    ...
    def set_positions(self, positions):
        ...
        self.action_thread = threading.Thread(
            target=self._execute_action, args=(positions,)
        )
